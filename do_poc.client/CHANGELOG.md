...
- Create Angular project with ng: `ng new do_poc.client --defaults --skip-install --skip-git `.
...
- Create project file (`do_poc.client.esproj`).
- Create `launch.json` to enable debugging.
- Create `nuget.config` to specify location of the JavaScript Project System SDK (which is used in the first line in `do_poc.client.esproj`).
